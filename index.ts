...
//Creating an interface named as Student which contains the attributes of a Student
interface Student {
    name : string,
    ID : number,
    course : string[],
...
const keys : string[] = Object.keys(courseFee);
...
while (true) {
    ...
    if (option.userChoice === "Add Student") {
        ...
        let randID : number = Math.floor((Math.random() * 100000) + 10000); 
        ...
        let totalFee : number = calculateTutionFee(studentDetails.course);
    ...
    var studentNames : string[] = arrayStudents.map(Student => Student.name);
    ...
    if (option.userChoice === "Update Student Info") {
        if (arrayStudents.length != 0) {
            ...
            switch (update.info) {
                ...
                case "Add Course":
                    let arrayCourses : string[] = ["Programming Fundamentals in Python     ----- $50", "Essentials of CyberSecurity            ----- $65", "Introduction to Machine Learning       ----- $35", "Data Science: Beginner to Advance      ----- $70", "Object Oriented Programming in Java    ----- $30"];
...
function calculateTutionFee(courses : string[]) : number {
    let fee : number = 0;
...
function showStudentDetails(student : Student) : void {
